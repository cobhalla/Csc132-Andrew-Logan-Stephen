...
class Game(Frame):
    def __init__(self, parent, controller):
        ...
        Game.currentMonster = self.randMonster()
        Game.text = ""
        label = Label(self, text = "Game")
        label.grid(row = 0, column = 0, sticky = NW )
        label1 = Label(self, text = Manager.player.name)
...
##        Game.text.insert(END, "hello")
    ...
    def randMonster(self):
        rint = randint(0,100)
        if Manager.player.kills < 50:
            if rint < 50:
                return Goblin()
            else:
                return RedGoblin()
        if Manager.player.kills >= 50 and Manager.kills < 100:
            if rint < 30:
                return Goblin()
...
##        Game.text.delete("1.0", END)
        if (Game.currentMonster == NONE):
            ...
            Game.text += str(Game.currentMonster) +\
                             "\n" + str(Manager.player) +\
                             "\nYou Have {} coins".format(Manager.player.coins) +\
                             "\nYou are carrying: " + str(Manager.player.inventory) +\
                             "\n\n" + status
            #Game.text.config(state = DISABLED)
            
    def attack(self):
        response = "You attack the monster"
        Game.currentMonster.health -= Manager.player.atk
...
class Shop(Frame):
    ...
    def __init__(self, parent, controller):
        ...
        label2 = Label(self, text = "You have {} coins.".format(Manager.player.coins))
    ...
    def buy(self, value):
        ...
        if value == "sword":
            ...
            if Manager.player.coins >= 10:
                response = "You buys a better sword"
                Manager.player.coins -= 10
                Manager.player.atk += 1
